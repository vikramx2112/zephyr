...
/*
 * @addtogroup t_randomnumgen_basic_api
 * @{
 * @defgroup t_random_get_entropy test_random_get_entropy
 * @brief TestPurpose: verify Random number generator works
 * @details
 * - Test Steps
 *   -# Read random numbers from RNG driver.
 *   -# Verify whether buffer overflow occured or not.
 *   -# Verify whether buffer completly filled or not.
 * - Expected Results
...
static int random_entropy(struct device *dev, char *buffer, char num)
{
	...
	memset((char *)buffer, (char)num, BUFFER_LENGTH);
...
void test_main(void)
{
	ztest_test_suite(randomnumgen_basic_api,
			ztest_unit_test(test_random_get_entropy));
	ztest_run_test_suite(randomnumgen_basic_api);
}
